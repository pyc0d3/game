...
class Generator():
    def __init__(self, width=64, height=64, max_rooms=20, min_room_xy=5,
                 max_room_xy=10, rooms_overlap=False, random_connections=2,
    ...
    def gen_level(self):
        ...
        for row in range(1, self.height - 1):
            for col in range(1, self.width - 1):
                if self.level[row][col] == 'floor':
                    self.floor_list.append((row, col))
        ...
        #additional set ups like player, door etc
        floor_id = random.randint(0, len(self.floor_list)//3)
        floor = self.floor_list[floor_id]
        self.level[floor[0]][floor[1]] = 'start_door'

        floor_id = random.randint(len(self.floor_list)*2//3, len(self.floor_list))
        floor = self.floor_list[floor_id]
        self.level[floor[0]][floor[1]] = 'exit_door'
    ...
    def gen_tiles_level(self):
        ...
        for row_num, row in enumerate(self.level):
            ...
            for col_num, col in enumerate(row):
                ...
                if col == 'player':
                    tmp_tiles.append(self.tiles['player'])
                if col == 'start_door':
                    tmp_tiles.append(self.tiles['start_door'])
                if col == 'exit_door':
                    tmp_tiles.append(self.tiles['exit_door'])
    ...
    def create_level_file(self, filename = 'map.txt'):
